...
### ✅ Task 0.1: 基本セットアップ
- `go mod init`でGoモジュール初期化
- `go get github.com/spf13/cobra`でCLIフレームワーク追加
- `go get github.com/yuin/goldmark`でMarkdownパーサー追加
- 基本ディレクトリ構造作成（`cmd/`, `internal/`）
...
- **(Red)** バージョン表示のテスト作成
- **(Green)** Cobraでバージョンコマンド実装
- **(Refactor)** コード整理

### ✅ Task 1.2: 標準入出力パススルー
**目標**: 標準入力の文字列をそのまま標準出力に返す
- **(Red)** パススルーのテスト作成
- **(Green)** 入力読み取り・出力ロジック実装
- **(Refactor)** コード整理
